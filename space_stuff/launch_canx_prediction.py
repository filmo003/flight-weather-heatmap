...
class launch_canx_predictor:
    ...
    criteria_dir = 'space_stuff/launch_criteria.yml' # Directory for rocket launch criteria
    weather_data_dir = 'space_stuff/combined_weather_data.csv' # Directory for hourly weather data 
    ...
    def calculate_cancellations(self, platform, site, start_window_date, end_window_date):
        ...
        site_data = self.weather_data.loc[self.weather_data['LOCATION_KEY']==site].sort_values(['YEAR', 'MONTH', 'DAY', 'HOUR'])
    ...
    def plot_hourly_data(data, platform, site, start_window_date, end_window_date):
        ...
        plt.xlim(np.nanmin(hourly_data['DATETIME']), np.nanmax(hourly_data['DATETIME']))
        plt.ylim(0, 105)
    ...
    def plot_daily_data(data, platform, site, year):
        ...
        plt.ylabel('Cancellation Rate')
        plt.xlim(np.nanmin(daily_data['DATETIME']), np.nanmax(daily_data['DATETIME']))
        plt.ylim(0, 105)
        plt.gca().yaxis.set_major_formatter(mticker.PercentFormatter())
...
if __name__ == '__main__':
    ...
    site = 'KVBG' # {'ZLJQ' :'Jiuquan Space Launch Center',
                     #  '53564':'Taiyuan Satellite Launch Center',
                     #  '59855':'Wenchang Satellite Launch Center',
                     #  '56571':'Xichang Satellite Launch Center',
                     #  '35849':'Baikonur Cosmodrome',
                     #  '22657':'Plesetsk Cosmodrome',
